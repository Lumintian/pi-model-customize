...
扩展注册了 `/model-customize` 命令（快捷别名 `/mc`），可在会话中随时查看当前配置与模型定制状态：

```text
/model-customize  # 或简写为 /mc
```

输出信息包括：
- **全局配置**：文件路径与加载状态（`loaded` / `not found`）。
- **项目配置**：文件路径、项目信任状态（未信任工作区会显式标记 `untrusted workspace, skipped`）与加载状态。
- **生效规则**：已解析合并的模式规则数和精确模型覆盖数。
- **当前模型状态**：当前模型标识、是否已应用定制、覆盖后的上下文窗口大小与最大 Token 上限、默认与允许的思考档位，以及当前活动思考强度。
...
本包遵循 [MIT License](./LICENSE) 开源许可证。
